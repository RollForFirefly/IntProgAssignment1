...
async def get_all_items(
    ...
    return await db_get_items(db, skip=skip, limit=limit)


@app.post("/items", response_model=Item)
async def create_item(item: Item, db: Session = Depends(get_session)):
    """Add a new task to the list."""
    db_item = await db_create_item(db, item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
async def update_item(
    ...
    db_item = await db_update_item(db, item_id, updated_object)
    if not db_item:
...
async def delete_item(item_id: str, db: Session = Depends(get_session)):
    ...
    db_item = await db_delete_item(db, item_id)
    if not db_item:
        raise HTTPException(status_code=404, detail="shop item not found")
    db.commit()
...
@app.get("/items/orders", response_model=List[Order])
async def get_all_orders(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_session)
):
    """Fetch the entire order list."""
    return await db_get_orders(db, skip=skip, limit=limit)


@app.post("/items/orders", response_model=Order)
async def create_order(order: Order, db: Session = Depends(get_session)):
    """Add a new order to the list."""
    db_order = await db_create_order(db, order)
    db.commit()
    db.refresh(db_order)
    return db_order


@app.put("/items/orders/{order_id}", response_model=Order)
async def update_order(
    order_id: str, updated_object: Order, db: Session = Depends(get_session)
):
    """Update an existing order by its ID."""
    db_order = await db_update_order(db, order_id, updated_object)
    if not db_order:
...
async def delete_order(order_id: str, db: Session = Depends(get_session)):
    ...
    db_order = await db_delete_order(db, order_id)
    if not db_order:
        raise HTTPException(status_code=404, detail="shop order not found")
    db.commit()
    # This code indicates the action was successful, the resource is gone, and no body content needs to be returned.
